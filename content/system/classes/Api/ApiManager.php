...
{    
    ...
    public function run() {
        ...
        if (!$this->cache_time || 'error' == $data['status'] || SYS_TIME - $this->cache_time > $data['timestamp']) {
            $this->send();
            $body = $this->response->resolve->getData();
            if ($body) {
                RC_Cache::app_cache_set($cache_key, array('body' => $body, 'status' => $this->response->resolve->getStatus(), 'timestamp' => SYS_TIME), 'system');
                return $body;
            } else {
                return false;
            }
    ...
    public function send() {
        ...
        $this->response->getResolve()->getData();
